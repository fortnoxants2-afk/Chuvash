...
const CategorySelector: React.FC<CategorySelectorProps> = ({ onSelectCategory, onBack, onNavigateToSettings, onNavigateToDice }) => {
      ...
      <div className="flex-1 w-full flex flex-col items-center justify-center px-[8.8%] relative overflow-visible">
        {categories.map((cat, index) => (
          <button
            ...
            style={{ 
              marginBottom: index < categories.length - 1 ? INTER_CARD_GAP : 0 
            }}
            className={`
              w-full aspect-[900/500] relative flex-shrink-0 outline-none
        ...
        {/* Кнопка кубика на правой плашке - Слегка уменьшена до w-[19%] */}
        <button 
          onClick={onNavigateToDice}
          className="absolute right-0 top-1/2 -translate-y-1/2 flex items-center justify-center rounded-l-[2.5rem] shadow-2xl active:scale-90 transition-all z-50 w-[19%] aspect-[1.15/1]"
          style={{ 
